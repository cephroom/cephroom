 ...
 * the free tier. Their subscription was fine; nobody was refunded; the
...
describe("when Stripe answers, entitlement is exactly what it said", () => {
  it("takes the tier Stripe reports, upward", () => {
    expect(tierOnRenewal({ fromStripe: "lab", current: "member" })).toBe("lab");
  });
  ...
  it("takes the tier Stripe reports, downward", () => {
    ...
    expect(tierOnRenewal({ fromStripe: "reader", current: "lab" })).toBe("reader");
  });

  it("takes reader when Stripe genuinely has no subscription", () => {
    expect(tierOnRenewal({ fromStripe: "reader", current: "reader" })).toBe("reader");
  });
});

describe("when Stripe does not answer, nothing is invented", () => {
  it("keeps a paying subscriber at the tier they hold", () => {
    expect(tierOnRenewal({ fromStripe: null, current: "lab" })).toBe("lab");
    expect(tierOnRenewal({ fromStripe: null, current: "member" })).toBe("member");
  });
  ...
  it("does not promote anybody during an outage", () => {
    ...
    expect(tierOnRenewal({ fromStripe: null, current: "reader" })).toBe("reader");
  });
  ...
  it("does not demote either, which was the bug", () => {
    ...
    expect(tierOnRenewal({ fromStripe: null, current: "lab" })).not.toBe("reader");
  });
  ...
  it("falls back to reader only when there is no current tier to carry", () => {
    ...
    // smallest one.
    expect(tierOnRenewal({ fromStripe: null, current: null })).toBe("reader");
  });
});
...
describe("both renewal paths share the policy", () => {
  it("is used by the refresh endpoint", async () => {
    ...
    expect(refresh).not.toMatch(/catch\s*\{\s*tier\s*=\s*"reader"/);
  });
