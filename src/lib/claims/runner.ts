...
export interface ClaimOutcome {
  ...
  evidence: { nPoints?: number | null; nDocs?: number | null; scope?: string } | null;
}

/** Reads one dataset cell. Returns null when nothing matches the query. */
export async function resolveCell(
  claim: Pick<ParsedClaim, "datasetSlug" | "metric" | "subject" | "object" | "scope">,
...
function pick(cell: ResolvedCell, select: ClaimSelect): {
  value: number | null;
  unit: string | null;
} {
  switch (select) {
...
export async function evaluateClaim(
  ...
  const cell = await resolveCell(claim);
...
/**
 * Re-parses a column body into claim rows, replacing whatever was stored.
 * Called whenever a draft is saved, so the claim table always matches the
 * text that is actually on the page.
 */
export async function syncClaims(columnId: string, body: string) {
...
/**
 * Runs every claim in a column and records the result. This is the CI job:
 * it is what publish calls, what the "Re-run checks" button calls, and what
 * a dataset re-import calls for every column that cites it.
 */
export async function runChecks(
  ...
  const conclusion = concludeRun(outcomes.map((o) => o.verdict));
  const counts = {
...
export async function latestCheckRun(columnId: string) {
